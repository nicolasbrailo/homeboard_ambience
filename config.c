...
#define CFG_GET_SZ(key) \
  if (!config_get_size_t(cfgbase, #key, &cfg->key)) { \
    fprintf(stderr, "Failed to read config: can't find size value " #key "\n"); \
    goto err; \
  }
#define CFG_GET_BOOL(key) \
  if (!config_get_bool(cfgbase, #key, &cfg->key)) { \
    fprintf(stderr, "Failed to read config: can't find bool value " #key "\n"); \
    goto err; \
  }
#define CFG_GET_STR(key) { \
    const char* tmp = NULL; \
    if (!config_get_string(cfgbase, #key, &tmp)) { \
      fprintf(stderr, "Failed to read config: can't find str value " #key "\n"); \
      goto err; \
    } else { \
...
#undef CFG_GET_STR
  ...
  if ((cfg->image_target_width < IMG_MIN_SIZE_PX) || (cfg->image_target_height < IMG_MIN_SIZE_PX)) {
    fprintf(stderr, "Config err: target width and height must be at least %dx%d\n", IMG_MIN_SIZE_PX, IMG_MIN_SIZE_PX);
    goto err;
  }
  ...
  if (cfg->shm_image_max_size_bytes < SHM_IMAGE_MAX_SIZE_BYTES_MIN) {
    fprintf(stderr, "Config err: shm_image_max_size_bytes of %zu is too small, min expected is %d\n", cfg->slideshow_sleep_time_sec, SHM_IMAGE_MAX_SIZE_BYTES_MIN);
    goto err;
  }

  if (cfg->shm_leak_file && !file_is_valid(cfg->shm_leak_image_path)) {
    fprintf(stderr, "Config err: shm_leak_image_path must point to a valid file, can't open %s\n", cfg->shm_leak_image_path);
    goto err;
  }
  ...
  if (cfg->slideshow_sleep_time_sec > SLIDESHOW_SLEEP_TIME_SEC_MAX) {
    fprintf(stderr, "Config err: slideshow_sleep_time_sec of %zu is too large, max expected is %d seconds\n", cfg->slideshow_sleep_time_sec, SLIDESHOW_SLEEP_TIME_SEC_MAX);
    goto err;
  }
