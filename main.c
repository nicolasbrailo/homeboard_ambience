...
void on_image_received(const void *img_ptr, size_t img_sz, const char *meta_ptr,
  ...
  g_img_render_pid = signal_single_kill_old(SIGUSR1, g_cfg->image_render_proc_name, g_img_render_pid);
...
int main(int argc, const char** argv) {
  ...
  if (!(g_shm = shm_init(g_cfg->shm_image_file_name, g_cfg->shm_image_max_size_bytes))) {
    goto err;
  }
  ...
  if (g_cfg->shm_leak_file) {
    if (shm_update_from_file(g_shm, g_cfg->shm_leak_image_path) <= 0) {
      fprintf(stderr, "Failed to update shm file with %s pre-shutdown, contents not defined\n", g_cfg->shm_leak_image_path);
    }
